...
def get_pie_chart(entered_site):
    ...
    if entered_site=='ALL':
        fig= px.pie(spacex_df, values='class', names='Launch Site', title='totaL successful launches per launch site')
        return fig
    else:
        site_df= filtered_df[filtered_df['Launch Site']==entered_site]
        outcome_counts= site_df['class'].value_counts().reset_index()
        outcome_counts.columns= ['class', 'count']
        fig = px.pie(outcome_counts, values='count', names='class', title=f'distribution of successful and unsuccessful launches for {entered_site}')
        return fig
# TASK 4:
